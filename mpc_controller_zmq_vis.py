...
class MPCControllerZMQ:
    ...
    def _solve_mpc(self) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            X_x_list = current_ref_traj['pos_x'].astype(float).tolist()
            X_y_list = current_ref_traj['pos_y'].astype(float).tolist()
        except Exception:
            pred_x_list = []
            pred_y_list = []
            X_x_list = []
            X_y_list = []
            print("[MPC] Failed to extract predicted trajectory.")

        return {
            "speed": v_cmd,
            "steering_angle": self.delta_f,
            "pred_x": pred_x_list,
            "pred_y": pred_y_list,
            "ref_x": X_x_list,
            "ref_y": X_y_list
            ...
            if now >= self._next_control_time:
                ...
                if self._global_path_ready and self._odom_ready:
                    ...
                    if cmd is not None:
                        ...
                        # primary control commands (speed and steering_angle),
                        # include the predicted x/y trajectory for visualisation
                        # by the ROS bridge.  If the predicted lists are empty
                        # or missing, the bridge will simply ignore them.
                        out: Dict[str, Any] = {
                            "type": "control",
                            "speed": cmd.get("speed", 0.0),
                            "steering_angle": cmd.get("steering_angle", 0.0),
                        }
                        # Pass through predicted trajectory if available
                        if isinstance(cmd.get("pred_x"), list) and isinstance(cmd.get("pred_y"), list):
                            out["pred_x"] = cmd["pred_x"]
                            out["pred_y"] = cmd["pred_y"]
                            out["ref_x"] = cmd["ref_x"]
                            out["ref_y"] = cmd["ref_y"]
                        try:
